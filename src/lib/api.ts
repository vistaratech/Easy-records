...
export async function addEntry(registerId: number, cells: Record<string, string> = {}, pageIndex: number = 0): Promise<Entry> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      await _syncAddRow(targetRegId, entry.id);
    }
...
export async function insertEntry(registerId: number, cells: Record<string, string> = {}, pageIndex: number = 0, atIndex: number): Promise<Entry> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of insertTargetRegIds) {
      await _syncInsertRow(targetRegId, atIndex, entry.id);
    }
...
async function _syncAddRow(targetRegisterId: number, sourceEntryId: number) {
  return runQueuedMutation(targetRegisterId, async () => {
    ...
    const newEntry: Entry = {
      ...
      cells: {
        "__sourceEntryId": sourceEntryId.toString()
      },
      createdAt: new Date().toISOString(),
      pageIndex: 0
    };
...
async function _syncDeleteRow(targetRegisterId: number, sourceEntryId: number, rowNumber: number) {
  return runQueuedMutation(targetRegisterId, async () => {
    const reg = await getRegDoc(targetRegisterId);
    let entryIndex = reg.entries.findIndex((e) => e.cells && e.cells["__sourceEntryId"] === sourceEntryId.toString());
    if (entryIndex === -1) {
      entryIndex = reg.entries.findIndex((e) => Number(e.rowNumber) === Number(rowNumber));
    }
...
async function _syncBulkDeleteRows(targetRegisterId: number, sourceEntryIds: number[], rowNumbers: number[]) {
  return runQueuedMutation(targetRegisterId, async () => {
    const reg = await getRegDoc(targetRegisterId);
    ...
    const entriesToDelete = reg.entries.filter(e => {
      ...
      return rowSet.has(Number(e.rowNumber));
    });
...
async function _syncReorderRows(targetRegisterId: number, sortedSourceEntryIds: number[]) {
  return runQueuedMutation(targetRegisterId, async () => {
    ...
    reg.entries.forEach(e => {
      const sId = e.cells?.["__sourceEntryId"];
      if (sId) entryMap.set(sId, e);
    });

    const newEntries: Entry[] = [];
    const usedIds = new Set<number>();

    sortedSourceEntryIds.forEach(id => {
...
export async function linkColumn(
  ...
  let sourceEntriesData: { rowNumber: number; entryId: number; value: string }[] = [];
  let sourceColName = '';
  let sourceColType = '';
...
export async function updateEntriesOrder(registerId: number, sortedEntries: Entry[]): Promise<void> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      await _syncReorderRows(targetRegId, sortedEntryIds);
    }
  });
}
...
export async function deleteEntry(registerId: number, entryId: number): Promise<void> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      await _syncDeleteRow(targetRegId, entry.id, entry.rowNumber);
    }
  ...
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      if (insertIndex === reg.entries.length) {
        ...
        await _syncInsertRow(targetRegId, insertIndex, entry.id);
      }
    }
    ...
    for (const { entry, index } of sorted) {
      ...
      for (const targetRegId of targetRegIds) {
        if (index >= 0 && index <= reg.entries.length) {
          await _syncInsertRow(targetRegId, index, entry.id);
        } else {
          await _syncAddRow(targetRegId, entry.id);
        }
      }
...
export async function duplicateEntry(registerId: number, entryId: number): Promise<Entry> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      await _syncAddRow(targetRegId, duplicate.id);
    }
...
export async function bulkDeleteEntries(registerId: number, entryIds: number[]): Promise<void> {
  return runQueuedMutation(registerId, async () => {
    ...
    for (const targetRegId of targetRegIds) {
      await _syncBulkDeleteRows(targetRegId, entryIds, deletedRowNumbers);
    }
